"use client";

import Image from "next/image";
import { motion } from "framer-motion";
import Link from "next/link";
import type { Insight } from "@/lib/content";

type NewsCardProps = {
  insight: Insight;
  isDarkMode?: boolean;
};

export default function NewsCard({ insight, isDarkMode }: NewsCardProps) {
  return (
    <motion.article
      className={`h-full border transition duration-300 rounded-xl overflow-hidden ${
        isDarkMode
          ? "border-white/10 hover:border-white/30 bg-[#1a1d26]"
          : "border-gray-200 hover:border-gray-900 bg-white"
      }`}
      whileHover={{
        boxShadow: isDarkMode
          ? "0 20px 40px -12px rgba(0, 0, 0, 0.5)"
          : "0 20px 40px -12px rgba(0, 0, 0, 0.15)",
        transition: {
          type: "spring",
          stiffness: 200,
          damping: 20,
        },
      }}
    >
      <Link href={`/news/${insight.slug}`} className="block h-full">
        <motion.div
          className="relative h-96 md:h-[28rem] w-full overflow-hidden"
          whileHover={{
            scale: 1.02,
          }}
          transition={{
            type: "spring",
            stiffness: 150,
            damping: 22,
          }}
        >
          <Image
            src={insight.coverImageUrl}
            alt={insight.title}
            fill
            className="object-cover"
            sizes="(max-width: 768px) 100vw, 33vw"
          />
          {/* Overlay gradient for better text readability */}
          <div className="absolute inset-0 bg-gradient-to-t from-black/20 to-transparent opacity-0 hover:opacity-100 transition-opacity duration-300" />
        </motion.div>

        <div className="p-5 md:p-6 min-h-[210px] md:min-h-[230px]">
          <div className="flex items-center justify-between gap-4 mb-4">
            <motion.span
              className={`inline-flex border px-2.5 py-1 text-[11px] font-medium rounded-md ${
                isDarkMode
                  ? "border-white/20 text-gray-300"
                  : "border-gray-300 text-gray-700"
              }`}
              whileHover={{
                scale: 1.05,
                backgroundColor: isDarkMode ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.05)",
              }}
              transition={{
                type: "spring",
                stiffness: 200,
                damping: 18,
              }}
            >
              {insight.category}
            </motion.span>
            <time
              className={`text-xs ${
                isDarkMode ? "text-gray-400" : "text-gray-500"
              }`}
            >
              {insight.date}
            </time>
          </div>

          <motion.h3
            className={`text-xl font-semibold tracking-tight leading-tight ${
              isDarkMode ? "text-white" : "text-gray-900"
            }`}
            whileHover={{
              x: 3,
            }}
            transition={{
              type: "spring",
              stiffness: 300,
              damping: 20,
            }}
          >
            {insight.title}
          </motion.h3>
          <p
            className={`mt-3 text-sm leading-relaxed line-clamp-2 ${
              isDarkMode ? "text-gray-400" : "text-gray-600"
            }`}
          >
            {insight.excerpt}
          </p>
        </div>
      </Link>
    </motion.article>
  );
}
